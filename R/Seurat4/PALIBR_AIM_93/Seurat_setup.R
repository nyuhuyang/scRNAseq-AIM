...
df_samples <- readxl::read_excel("output/20220519/20220318_scRNAseq_info.xlsx")
df_samples = as.data.frame(df_samples)
colnames(df_samples) %<>% tolower()
df_samples$date %<>% gsub(" UTC","",.) %>% as.character()
df_samples$sample %<>% gsub("AIM17_EOT","AIM17_64",.)

#======1.2 load  Seurat =========================
object = readRDS(file = "data/MCL_AIM_93_20220519.rds")
object$orig.ident %<>% gsub("AIM17_EOT","AIM17_64",.)
...
saveRDS(object, file = "data/MCL_AIM_93_20220519.rds")
...
object %<>% RunUMAP(reduction = "pca", dims = 1:npcs,  densmap = TRUE,
                    min.dist = 0.4,spread = 0.6,
                    return.model = TRUE)
object[["raw.densumap"]] <- CreateDimReducObject(embeddings = object@reductions[["umap"]]@cell.embeddings,
                                             key = "rawdensUMAP_", assay = DefaultAssay(object))
colnames(object[["raw.densumap"]]@cell.embeddings) %<>% paste0("raw-dens",.)
saveRDS(object, file = "data/MCL_AIM_93_20220519.rds")
...
object %<>% RunUMAP(reduction = "harmony", dims = 1:npcs,  densmap = TRUE,
                    min.dist = 0.4,spread = 0.6,
                    return.model = TRUE)
object[["dens.umap"]] <- CreateDimReducObject(embeddings = object@reductions[["umap"]]@cell.embeddings,
                                             key = "densUMAP_", assay = DefaultAssay(object))
colnames(object[["dens.umap"]]@cell.embeddings) %<>% paste0("dens-",.)
...
saveRDS(object, file = "data/MCL_AIM_SCT_93_20220519.rds")
