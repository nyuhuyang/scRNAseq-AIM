...
if(reference == "MCL+blue_encode"){
    pred = readRDS("output/MCL_AIM_93_20220519_blue_encode_singleR_pred.rds")
    ...
    singlerDF$label1.blue_encode %<>% gsub("Eosinophils|Megakaryocytes","other Myeloid cells",.)
...
if(reference == "MCL+blue_encode"){
    pred = readRDS("output/MCL_AIM_93_20220519_azimuth_PBMC_singleR_pred.rds")
    singlerDF$celltype.l3 =  pred$pruned.labels
    table(rownames(pred) == rownames(object@meta.data))
    ...
    meta.data = read.csv(paste0(FilePath,"GSE164378/GSE164378_sc.meta.data_5P.csv"),row.names =1)
    meta.data = meta.data[!duplicated(meta.data$celltype.l3),]
    
    singlerDF$celltype.l2 = plyr::mapvalues(singlerDF$celltype.l3,
                                            from = meta.data$celltype.l3, to = meta.data$celltype.l2)
    singlerDF$celltype.l1 = plyr::mapvalues(singlerDF$celltype.l3,
                                            from = meta.data$celltype.l3, to = meta.data$celltype.l1)
    # reduce false positive results (B cells are labeled as MCL in normal samples)
    # and false negative results (MCL cells are labeled as B cells in MCL samples)
    # singler1sub false negative results  =========
    CCND1 = FetchData(object,"CCND1")
...
saveRDS(object, file = "MCL_AIM_93_20220519.rds")
